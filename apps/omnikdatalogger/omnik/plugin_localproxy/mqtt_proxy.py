...
class MQTTproxy(LocalProxyPlugin):
    ...
    def _mqtt_on_connect(self, client, userdata, flags, rc=0, properties=None):
        if rc == 0:
            hybridlogger.ha_log(
                self.logger, self.hass_api, "INFO", "MQTTproxy client connected"
            )
            # subscribe to datalogger objects
            for inverter in self.client.inverters:
                topic = (
                    f"{self.mqtt_discovery_prefix}/binary_sensor/{self.logger_sensor_name}_"
                ...
                try:
                    ...
                    hybridlogger.ha_log(
                        ...
                        self.hass_api,
                        "INFO",
                        f"MQTTproxy subscribed on topic: {topic}",
                    )
                    ...
                    hybridlogger.ha_log(
                        ...
                        self.hass_api,
                        "ERROR",
                        f"MQTTproxy failed to subsrcibed ontopic: {topic}. Error {e}",
                    )
    ...
    def _mqtt_on_message(self, client, userdata, message):
        ...
        try:
            ...
            hybridlogger.ha_log(
                ...
                self.hass_api,
                "WARNING",
                f"MQTTproxy: invalid data received on topic: {message.topic}. Error {e}",
            )
